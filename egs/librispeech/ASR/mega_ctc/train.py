...
def compute_loss(
    ...
    feature = batch["inputs"]  #torch.Size([10, 2000, 80])
    ...
    ctc_loss = k2.ctc_loss(
        ...
        reduction=params.reduction,  #'sum'
        use_double_scores=params.use_double_scores,  #true
    )

    if params.att_rate != 0.0:  #0.8
        with torch.set_grad_enabled(is_training):
            mmodel = model.module if hasattr(model, "module") else model  #false
    ...
    info = MetricsTracker()
    info["frames"] = supervision_segments[:, 2].sum().item()  #4649
    ...
    info["utterances"] = feature.size(0)  #10
    # averaged input duration in frames over utterances
    info["utt_duration"] = supervisions["num_frames"].sum().item()  #18603   4649*4=18596
    # averaged padding proportion over utterances
    info["utt_pad_proportion"] = (
        ((feature.size(1) - supervisions["num_frames"]) / feature.size(1)).sum().item()  # ()里头tensor([0.0000, 0.0535, 0.0540, 0.0755, 0.0840, 0.0980, 0.0630, 0.0715, 0.0960,0.1030])
    )  #0.6984999775886536
...
def run(rank, world_size, args):
    ...
    """
    params = get_params()
    params.update(vars(args))  # 来自command line的会覆盖params里的默认值
    ...
    lexicon = Lexicon(params.lang_dir)  #没看呢
    max_token_id = max(lexicon.tokens)  #499
    num_classes = max_token_id + 1  # +1 for the blank #500
